package icu.mabbit.rlg.common.interceptor;
        ...
        implements Interceptor
{
            ...
            throws Throwable
    {
        ...
        switch (getOriginalSqlType(sql))
        {
            case SQL_TYPE_INSERT:
                log.debug("原SQL为【INSERT】语句，准备补充创建与更新时间...");
                newSql = appendCreateTimeField(sql, LocalDateTime.now());
                break;
            case SQL_TYPE_UPDATE:
                log.debug("原SQL为【UPDATE】语句，准备补充更新时间...");
                newSql = appendModifiedTimeField(sql, LocalDateTime.now());
                break;
            case SQL_TYPE_OTHER:
                log.debug("原SQL不为【INSERT】huo【UPDATE】语句，不补充时间，直接放行");
                break;
        }

        if (newSql != null)
        {
            log.debug("新SQL：{}", newSql);
    ...
    private String appendCreateTimeField(String sqlStatement, LocalDateTime dateTime)
    {
        ...
        {
            log.debug("原SQL中包含create_time，不进行补充");
            return null;
        }
    ...
    private String appendModifiedTimeField(String sqlStatement, LocalDateTime dateTime)
    {
        ...
        {
            log.debug("原SQL中包含modified_time，不进行补充");
            return null;
        }
